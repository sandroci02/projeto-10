...
import { PageEvent, MatPaginator, MatDialog, Sort } from '@angular/material';
...
export class UsuarioComponent implements OnInit {
  ...
  openDialogExcluir(selecionado: Usuario): void {
    const dialogRef = this.dialog.open(ConfirmacaoComponent, {
      width: '450px',
      data: {
        titulo: 'Tem certeza?', mensagem: 'Excluir Usuário ' + selecionado.nome
      }
    });
  ...
  openDialogReset(selecionado: Usuario): void {
    const dialogRef = this.dialog.open(ConfirmacaoComponent, {
      width: '450px',
      data: {
        titulo: 'Tem certeza?', mensagem: 'Resetar a senha do Usuário ' + selecionado.nome
      }
    });
